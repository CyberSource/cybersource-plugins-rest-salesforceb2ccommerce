...
describe('Payment', function () {
    ...
    it('should take payment Methods and convert to a plain object ', function () {
        var result = new PaymentModel(createApiBasket({
    ...
    it('should take payment instruments and convert to a plain object ', function () {
        ...
        assert.equal(result.selectedPaymentInstruments[2].paymentMethod, 'DW_GOOGLE_PAY');
        assert.equal(result.selectedPaymentInstruments[2].amount, 0);
        assert.equal(result.selectedPaymentInstruments[2].type, 'Visa');
        assert.equal(
            result.selectedPaymentInstruments[2].maskedCreditCardNumber,
            '************1111'
        );
    });
});
